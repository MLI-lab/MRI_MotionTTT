...
if __name__ == '__main__':
    ...
    results_dir = "motionTTT_recons"
    path_to_result_dir = f"{data_drive}cc-359_raw/calgary-campinas_version-1.0/CC359/Raw-data/Multi-channel/12-channel/motion_MRI_results"
        ...
        for example_dict in examples_list_of_dicts:
            ...
            for max_mot in [2,5,10]:
                for random_motion_seed_ind in ['seed1', 'seed2']: 
                    ...
                    args.TTT_states_per_split = 1 # set here 1 for inter- and 10 for intra-shot motion
                    ...
                    # For intra-shot
                    #args.experiment_name_TTT= f"_rottrans{max_mot}_Ns{args.Ns}_motEvents{num_motion_events}_intraEvents{args.num_intraShot_events}_motSeed{random_motion_seed}_splits{args.TTT_states_per_split}"
                    # For inter-shot
                    args.experiment_name_TTT= f"_rottrans{max_mot}_Ns{args.Ns}_motEvents{num_motion_events}_motSeed{random_motion_seed}_splits{args.TTT_states_per_split}"
                    ...
                    if args.L1min:
                        for L1min_DC_loss_thresholding in [True, False]:
                            for L1min_motion_alignment in [True]:
                                for phase in [0,1,2]:
                                    try:                                                
                                        args.Ns = 50
